function rectangularAreaChartDefaultSettings(){
    return {
        ...
        colorsScale: d4.scale.category20b(), // The color scale to use for the chart areas.
...
function loadRectangularAreaChart(elementId, data, settings){
    ...
    var dataMin = Math.min(data[0].value, settings.minValue);

    var width = parseInt(svg.style("width"));
    var height = parseInt(svg.style("height"));

    // Scales for the height and width of the boxes.
    var sizeScaleWidth = d4.scale.sqrt().range([0, width]).domain([dataMin-5, dataMax]);
    var sizeScaleHeight = d4.scale.sqrt().range([0, height]).domain([dataMin-5, dataMax]);
